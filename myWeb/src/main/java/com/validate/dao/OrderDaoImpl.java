...
public class OrderDaoImpl implements OrderDao{
    ...
    public Order findOrderById(String id) throws SQLException {
        String sql = "select * from orders,userinfo where orders.user_id=userinfo.id and orders.id=?";
        QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
        return runner.query(sql, new ResultSetHandler<Order>() {
            public Order handle(ResultSet rs) throws SQLException {
                ...
                while (rs.next()) {
                    ...
                    order.setOrdertime(rs.getTimestamp("orders.ordertime"));
                    ...
                    user.setEmail(rs.getString("userinfo.email"));
                    user.setCode(rs.getString("userinfo.code"));
                    user.setPassword(rs.getString("userinfo.password"));
                    user.setRole(rs.getString("userinfo.role"));
                    user.setActivate_state(rs.getInt("userinfo.activate_state"));
                    user.setUsername(rs.getString("userinfo.username"));
                    order.setUser(user);
                }
                return order;
            }
    ...
    public List<Order> findAllOrder() throws SQLException {
        //1.创建sql
        String sql = "select orders.*,userinfo.* from orders,userinfo where userinfo.id=orders.user_id order by orders.user_id";
        //2.创建QueryRunner对象
        QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
    ...
    public List<Order> findOrderByManyCondition(String id, String receiverName)
        ...
        //2.定义查询sql
        String sql = "select orders.*,userinfo.* from orders,userinfo where userinfo.id=orders.user_id ";
        //3.根据参数拼接sql语句
