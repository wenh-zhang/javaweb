...
<main class="main">
    <div id="cartBox">
        <form  id="orderForm" style="width: 100%;height: 100%" action="<%=path%>/com.servlet.user/SubmitOrderServlet" method="post">

            <div class="receiverinfo">
                <div>
                    <input name="receiverName" id="receiverName" placeholder="收货人">
                    <input name="receiverPhone" id="receiverPhone" placeholder="联系电话">
                    <input name="receiverAddress" id="receiverAddress" placeholder="收货地址">
            ...
            <div class="cartBox1" style="height: 50%">
                <table class="cart" border="1" cellspacing="0" cellpadding="0"
                    ...
                    <c:forEach items="${waitorder}" var="w" varStatus="vs">
                        <tr>
                            <td style="width: 10%">${vs.count}</td>
                            <td style="width: 15%">${w.key.id}</td>
                            <td style="width: 10%"><img style="margin: 0px auto;width: auto;height: 60px" src="${pageContext.request.contextPath}${w.key.imgurl}"  ></td>
                            <td style="width: 15%"><a href="<%=path%>/com.servlet.common/ViewProductServlet?id=${w.key.id}">${w.key.name}</a></td>
                            <td style="width: 10%">${w.key.price}</td>
                            <td style="width: 15%">${w.value}</td>
                            <td style="width: 12%">${w.key.price*w.value}</td>
                        </tr>
                    </c:forEach>
            ...
            <div class="cartBox2">
                <div style="height: 80%;width: 100%;padding-left: 50%;font-size: 20px">
                    <div style="width: 100%;display: inline-block">
                        <a>商品总价（不含运费）: </a><a type="text" id="all">${sum}</a><a> 元</a>
                    </div>
